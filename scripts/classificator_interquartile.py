...
'''''
improves classificator.py using also iqr
'''''
                ...
                if not df_chunk.empty:
                    ...
                    if df_chunk.index.max() + pd.Timedelta(milliseconds=5) > (pd.Timestamp(year=current_day.year,
                        ...
                        df_windowed = pd.DataFrame(
                            columns=['datetime', 'acc_dev_std', 'iqr', 'activity'])  # df with new features
                        sleep_hours = []
                        awakenings = 0
                        awake = True
                        ...
                        while start_time <= (df.index.max() - window_current):  # as long as we can compute a window of data
                            ...
                            # (if needed must be inserted in df_windowed, saved in c30 and plotted)

                            # STANDARD DEVIATION OF ACCELERATIONS NORM
                            acc_dev_std = df_in_window['norm'].std()

                            # STANDARD DEVIATION OF ARM ANGLE
                            # ang_dev_std = df_in_window['angle'].std()

                            # INTER QUARTILE RANGE:
                            norm_array = df_in_window['norm'].to_numpy()
                            q3, q1 = np.percentile(norm_array, [75, 25])
                            iqr = q3 - q1
                            ...
                            if awake:  # if was considered awake
                                ...
                                if iqr <= iqr_lim and acc_dev_std <= acc_dev_std_lim:  # -> maybe fell asleep
                                    # print(str(df_in_window.index.min()) + " (AWAKE) feature <= limit --> MAYBE ASLEEP")
                                    if stopped_moving_at is None:  # was considered moving until now
                                        ...
                                        if (df_in_window.index.min() - stopped_moving_at) >= interval_for_asleep:
                                            ...
                                            # add new row in df_windowed with associated features computed in the window
                                            df_windowed.loc[len(df_windowed.index)] = [df_in_window.index.min(),
                                                                                       acc_dev_std, iqr, "Awake"]
                                else:  # was awake and is still considered awake
                                    # print("(AWAKE) feature > limit --> ACTUALLY STILL AWAKE, reset stopped_moving_at")
                                    stopped_moving_at = None
                                    # add new row in df_windowed with associated features computed in the window
                                ...
                                if iqr > iqr_lim and acc_dev_std > acc_dev_std_lim:  # maybe woke up
                                    # print(str(df_in_window.index.min()) + " (ASLEEP) feature > limit --> MAYBE AWAKE")
                                    ...
                                    # print("(ASLEEP) feature <= limit --> ACTUALLY STILL ASLEEP, reset started_moving_at")
                                    started_moving_at = None
                                    # add new row in df_windowed with associated features computed in the window
                                    df_windowed.loc[len(df_windowed.index)] = [df_in_window.index.min(), acc_dev_std,
                                                                               iqr, "Asleep"]
                        ...
                        c30 = df_windowed[['acc_dev_std', 'iqr', 'activity']]
                        if not os.path.isfile(filepath):
                            c30.to_csv(filepath)
                        else:
